...
import Timeline, { TimelineItemData } from "@/components/visualization/Timeline/Timeline";
...
const ExperienceSection: FunctionComponent<Props> = (props) => {
  ...
  const timelineData = useMemo<TimelineItemData[]>(() => {
    ...
    const timelineData = experiencesData.reduce((acc: TimelineItemData[], currNode) => {
      ...
      const currTimelineData: TimelineItemData = {
        ...
        endDate: (currData.frontmatter.endDate) ? new Date(currData.frontmatter.endDate) : null,
    ...
    >
      <ContentWrapper wideness="wide">
        <div className="ExperienceSection__content">
          ...
          <GradientDivider className="ExperienceSection__divider" gradientFade="left-right" />
          <Timeline className="ExperienceSection__timeline" timelineData={timelineData} />
        </div>
        {/* <div
